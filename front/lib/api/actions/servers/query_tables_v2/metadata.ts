...
export const QUERY_TABLES_V2_TOOLS_METADATA = createToolsRecord({
  [LIST_TABLES_TOOL_NAME]: {
    description:
      "List and discover the structured data tables, datasets, and table URIs available to this agent. " +
      "Returns lightweight table metadata and URIs. Call this first to find available tables, then pass selected URIs to get_database_schema.",
    schema: {
      tables:
  ...
  [GET_DATABASE_SCHEMA_TOOL_NAME]: {
    description:
      "Inspect the database schema and table structure for selected tables before writing SQL. Use this to answer what columns, fields, column names, data types, sample rows, and relationships exist. " +
      "You MUST call list_tables first to discover available tables, then call this tool with the URIs of the tables you need before running SQL.",
    schema: {
  ...
  [EXECUTE_DATABASE_QUERY_TOOL_NAME]: {
    description:
      "Run and execute SQL against selected structured data tables to analyze, aggregate, filter, join, or export result rows. " +
