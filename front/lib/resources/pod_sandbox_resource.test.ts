...
describe("PodSandboxResource", () => {
  it("creates a sandbox and pod ownership row", async () => {
    ...
    const result = await PodSandboxResource.ensureSandboxActive(
      authenticator,
      pod
    );
  ...
  it("deleteSandbox deletes the owner link and sandbox row", async () => {
    ...
    const pod = await SpaceFactory.project(workspace);
    const createResult = await PodSandboxResource.ensureSandboxActive(
      authenticator,
      pod
    );
    ...
    const deleteResult = await PodSandboxResource.deleteSandbox(
      authenticator,
      pod
    );
