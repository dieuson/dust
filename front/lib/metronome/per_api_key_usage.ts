 ...
 * query is capped server-side and silently omits groups. Free usage is excluded
 * by dropping `usage_type === "free"` buckets in code. AI Usage is already AWU
 * spend (cost_awu, priced 1:1); Tool Usage is an invocation count weighted by
 * the per-category AWU price.
 */
export async function fetchPerApiKeyAwuUsage({
  ...
  const [aiResult, toolResult] = await Promise.all([
    fetchSegmentedUsage({
      ...
      groupKey: [API_KEY_NAME_GROUP_KEY, USAGE_TYPE_GROUP_KEY],
      keyNames,
    }),
    fetchSegmentedUsage({
      segments,
      ...
      groupKey: [API_KEY_NAME_GROUP_KEY, USAGE_TYPE_GROUP_KEY, "tool_category"],
      keyNames,
    }),
  ]);
  if (aiResult.isErr()) {
