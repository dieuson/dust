...
// `metronome:api_key_credit_state:<ws>:<keyId>` holds the per-key credit state
...
function buildApiKeyCreditStateKey(
  ...
  return `metronome:api_key_credit_state:${workspaceId}:${keyId}`;
}
...
export async function setApiKeyCreditState(
  ...
  await runOnRedis({ origin: REDIS_ORIGIN }, async (client) => {
    await client.set(buildApiKeyCreditStateKey(workspaceId, keyId), state);
  });
}
...
export async function getApiKeyCreditState(
  ...
  const cached = await runOnRedis({ origin: REDIS_ORIGIN }, async (client) =>
    client.get(buildApiKeyCreditStateKey(workspaceId, keyId))
  );
  ...
  logger.info(
    { workspaceId, keyId, apiKeyCreditStateCacheHit: false },
  ...
  await setApiKeyCreditState(workspaceId, keyId, state);
  return state;
}
...
export async function isApiKeyCapped(
  ...
  const state = await getApiKeyCreditState(workspaceId, keyId);
  return state === "capped";
}
