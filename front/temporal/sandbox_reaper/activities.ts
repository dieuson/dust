...
import { PodSandboxResource } from "@app/lib/resources/pod_sandbox_resource";
...
type SandboxOwnerRef = {
  kind: SandboxLifecycleOwner["kind"];
  modelId: ModelId;
};

type SandboxOwnerMaps = {
  ownerRefsBySandboxModelId: Map<ModelId, SandboxOwnerRef>;
  ownersBySandboxModelId: Map<ModelId, SandboxLifecycleOwner>;
};
...
/**
 * Fetch the owner resource for each sandbox. The reaper spans every workspace,
...
async function fetchSandboxOwnerMaps(
  ...
  const podModelIdsBySandboxModelId =
    await PodSandboxResource.dangerouslyFetchPodModelIdsBySandboxes(sandboxes);
  ...
  const ownersBySandboxModelId = new Map<ModelId, SandboxLifecycleOwner>();
  ...
  for (const sandbox of sandboxes) {
    ...
    if (pod) {
      ownersBySandboxModelId.set(sandbox.id, {
        ...
        modelId: pod.id,
        dangerouslyDestroySandboxIfKillRequested: (auth) =>
          PodSandboxResource.dangerouslyDestroySandboxIfKillRequested(
            auth,
            pod
          ),
        dangerouslyDestroySandboxIfSleeping: (auth) =>
          PodSandboxResource.dangerouslyDestroySandboxIfSleeping(auth, pod),
        dangerouslySleepSandboxIfPendingApproval: (auth) =>
          PodSandboxResource.dangerouslySleepSandboxIfPendingApproval(
            auth,
            pod
          ),
        dangerouslySleepSandboxIfRunning: (auth) =>
          PodSandboxResource.dangerouslySleepSandboxIfRunning(auth, pod),
      });
    }
  }
 ...
 * owner resource for each sandbox, then run `action` concurrently. The
 * lifecycle methods run from the owner resource so callers do not need to know
